# /refactor - 모듈화 및 타입 가이드라인 검사 (project)
...
**규칙:**
- 공유 타입 → `@auction/shared/types` (packages/shared/src/types/)
...
```
@auction/shared (최하위) ← @auction/ui ← src/lib ← features/* ← app/* (최상위)
```
