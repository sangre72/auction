...
import { boardsApi, postsApi, type Board, type Post, type PostUpdate } from '@/lib/api';
...
export default function EditPostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await postsApi.update(boardSlug, postId, data);
      ...
      if (apiError?.status === 401) {
        ...
        setError(apiError?.detail || '게시글 수정에 실패했습니다.');
      }
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <Link href={`/board/${boardSlug}/${postId}`} className="hover:text-purple-600 transition-colors truncate max-w-xs">
          {post.title}
        </Link>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
